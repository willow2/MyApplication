...
public class RollerView extends View {
    ...
    /** 当前条目中的文字颜色 */
    private static final int VALUE_TEXT_COLOR = Color.rgb(100, 100, 100);

    /** 非当前条目的文字颜色 */
    private static final int ITEMS_TEXT_COLOR = Color.rgb(179, 179, 179);
    ...
    /** WheelView 适配器 */
    private RollerAdapter adapter = null;
    /** 当前显示的条目索引 */
    ...
    private List<OnRollerChangedListener> changingListeners = new LinkedList<OnRollerChangedListener>();
    /** 条目滚动监听器集合, 该监听器封装了 开始滚动方法, 结束滚动方法 */
    private List<OnRollerScrollListener> scrollingListeners = new LinkedList<OnRollerScrollListener>();
     ...
     * 构造方法
     */
    public RollerView(Context context, AttributeSet attrs, int defStyle) {
     ...
     * 		返回适配器
     */
    public RollerAdapter getAdapter() {
        return adapter;
    }
     ...
     *            the listener
     */
    public void addChangingListener(OnRollerChangedListener listener) {
     ...
     *            the listener
     */
    public void removeChangingListener(OnRollerChangedListener listener) {
    ...
    protected void notifyChangingListeners(int oldValue, int newValue) {
        for (OnRollerChangedListener listener : changingListeners) {
     ...
     *            the listener
     */
    public void addScrollingListener(OnRollerScrollListener listener) {
     ...
     *            the listener
     */
    public void removeScrollingListener(OnRollerScrollListener listener) {
    ...
    protected void notifyScrollingListenersAboutStart() {
        for (OnRollerScrollListener listener : scrollingListeners) {
        	//回调开始滚动方法
    ...
    private int getMaxTextLength() {
        RollerAdapter adapter = getAdapter();
        if (adapter == null) {
            return 0;
        }
    ...
    private void createLayouts(int widthItems, int widthLabel) {
        ...
        if (itemsLayout == null || itemsLayout.getWidth() > widthItems) {
            ...
            itemsLayout = new StaticLayout(buildText(isScrollingPerformed), itemsPaint, widthItems,
                    widthLabel > 0 ? Layout.Alignment.ALIGN_OPPOSITE : Layout.Alignment.ALIGN_CENTER,1.5f,
        ...
        if (widthLabel > 0) {
            if (labelLayout == null || labelLayout.getWidth() > widthLabel) {
                labelLayout = new StaticLayout(label, valuePaint, widthLabel, Layout.Alignment.ALIGN_NORMAL, 1.5f,
                        ADDITIONAL_ITEM_HEIGHT, false);
            } else {
    ...
    private void drawCenterRect(Canvas canvas) {
		...
		int offSet = (int) (getItemHeight() / 3);
